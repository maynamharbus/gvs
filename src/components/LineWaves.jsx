...
export default function LineWaves({
  ...
  useEffect(() => {
    ...
    const useMobileMotion = enableMouseInteraction && isCoarsePointer;
    ...
    function setTargetPosition(clientX, clientY) {
      ...
      targetMouse = [
        clamp((clientX - rect.left) / rect.width, 0.0, 1.0),
        clamp(1.0 - (clientY - rect.top) / rect.height, 0.0, 1.0)
      ];
    }
    ...
    function update(time) {
      ...
      if (enableMouseInteraction) {
        if (useMobileMotion && time - lastTouchTimestamp > 1400) {
          const loopT = time * 0.00018;
          const scrollMax = Math.max(document.documentElement.scrollHeight - window.innerHeight, 1);
          const scrollProgress = scrollMax > 1 ? window.scrollY / scrollMax : 0.5;
          const driftX = 0.5 + Math.cos(loopT * 1.9) * 0.22 + Math.sin(loopT * 0.7) * 0.05;
          const driftY = 0.2 + scrollProgress * 0.58 + Math.sin(loopT * 1.15) * 0.07;
          targetMouse[0] = clamp(driftX, 0.08, 0.92);
          targetMouse[1] = clamp(driftY, 0.08, 0.92);
        }
